...
function PersonCard({ person, type }) {
  return (
    ...
    >
      ...
      <h3 className="text-xl font-semibold text-acePurple">{person.full_name}</h3>
      <p className="text-gray-600 mt-1">
        {type === "mentor" ? person.expertise_area : person.field_of_interest}
      </p>
      {person.bio && (
        <p className="text-gray-700 mt-2 line-clamp-3">{person.bio}</p>
      )}
    </motion.div>
  );
}
  ...
  return (
    <section className="bg-aceLight py-16 px-4">
      <div className="max-w-7xl mx-auto text-center">
        <motion.h1
          className="text-4xl md:text-5xl font-bold text-acePurple mb-6"
        ...
        {loading && <p className="text-gray-500">Loading mentors and mentees...</p>}
        {errorMsg && <p className="text-red-600">{errorMsg}</p>}

        {/* Mentors Section */}
        {!loading && mentors.length > 0 && (
          <div className="mb-12 text-left">
            <h2 className="text-2xl font-semibold text-acePurple mb-4">Mentors</h2>
            <input
              type="text"
              placeholder="Search mentors by name or expertise..."
              className="w-full md:w-1/2 mb-6 px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-acePurple"
              value={mentorSearch}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              {filteredMentors.length > 0 ? (
                ...
                <p className="text-gray-500 col-span-full">No mentors match your search.</p>
              )}
            </div>
          </div>
        )}
        ...
        {!loading && mentees.length > 0 && (
          <div className="text-left">
            <h2 className="text-2xl font-semibold text-acePurple mb-4">Mentees</h2>
            <input
              type="text"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              {filteredMentees.length > 0 ? (
                ...
                <p className="text-gray-500 col-span-full">No mentees match your search.</p>
              )}
            </div>
          </div>
        )}
